...
rule skip_pilon:
  input:
    asm = lambda wildcards: __NOCASE__ if tconfig[wildcards.assembler]["pilon_do"] else "%s/racon.%s.%s.fa" % (RACON_OUTDIR, wildcards.assembler, wildcards.sample_id)
  output:
...
rule pilon_begin:
  input:
    asm = lambda wildcards: "%s/racon.%s.%s.fa" % (RACON_OUTDIR, wildcards.assembler, wildcards.sample_id) if tconfig[wildcards.assembler]["pilon_do"] else __NOCASE__
  output:
...
rule pilon_end:
  input:
    asm = lambda wildcards: expand("%s/pilon.%s.%s/asm.{iter}.fa" % (PILON_OUTDIR, wildcards.assembler, wildcards.sample_id), iter=range(1, tconfig[wildcards.assembler]["pilon_maxiter"] + 1))
  output:
    asm = "%s/pilon.{assembler}.{sample_id}.fa" % (PILON_OUTDIR)
  params:
    final_asm = lambda wildcards: "%s/pilon.%s.%s/asm.%d.fa" % (PILON_OUTDIR, wildcards.assembler, wildcards.sample_id, tconfig[wildcards.assembler]["pilon_maxiter"])
...
rule pilon_iter_polish:
  ...
  params:
    ...
    max_iter     = lambda wildcards: tconfig[wildcards.assembler]["pilon_maxiter"],
    pilon_maxmem = lambda wildcards: tconfig[wildcards.assembler]["pilon_maxmem"],
