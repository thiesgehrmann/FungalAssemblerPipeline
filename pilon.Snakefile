...
if not(tconfig["pilon"]["do"]):
  ...
  rule pilon_end:
    input:
      asm = lambda wildcards: expand("%s/pilon.%s.%s/asm.{iter}.fa" % (PILON_OUTDIR, wildcards.assembler, wildcards.sample_id), iter=range(1, tconfig["pilon"]["maxiter"] + 1))
    output:
      asm = "%s/pilon.{assembler}.{sample_id}.fa" % (PILON_OUTDIR)
    params:
      final_asm = lambda wildcards: "%s/pilon.%s.%s/asm.%d.fa" % (PILON_OUTDIR, wildcards.assembler, wildcards.sample_id, tconfig["pilon"]["maxiter"])
  ...
  rule pilon_iter_polish:
    ...
    params:
      ...
      max_iter   = tconfig["pilon"]["maxiter"],
      pilon_maxmem = tconfig["pilon"]["maxmem"],
