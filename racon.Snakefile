...
if not(tconfig["racon"]["do"]):
  ...
  rule racon_end:
    input:
      asm   = lambda wildcards: expand("%s/racon.%s.%s/asm.{iter}.fa" % (RACON_OUTDIR, wildcards.assembler, wildcards.sample_id), iter=range(1, tconfig["racon"]["maxiter"] + 1))
    output:
      asm = "%s/racon.{assembler}.{sample_id}.fa" % (RACON_OUTDIR)
    params:
      final_asm = lambda wildcards: "%s/racon.%s.%s/asm.%d.fa" % (RACON_OUTDIR, wildcards.assembler, wildcards.sample_id, tconfig["racon"]["maxiter"])
  ...
  rule racon_iter_polish:
    ...
    params:
      ...
      max_iter   = tconfig["racon"]["maxiter"],
